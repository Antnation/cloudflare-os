...
describe("Google Doc tab isolation", () => {
  ...
  it("drops an edit whose tab is deleted before approval", async () => {
    let docs = nestedDocs();
    let actionId = await hooks().submitAppend("tabs-deleted", "added", "metrics");

    docs.removeTab("metrics");
    expect(await hooks().applyAction("tabs-deleted", actionId)).toBeNull();
...
describe("Google Doc edits stored before tab support", () => {
  ...
  it("refuses a record naming no tab even when the document has exactly one", () => {
    expect(() => googleDocActionTab(snapshot, storedAppend)).toThrow(
